...
# Dependency graph
requires:
  - "lib/presentation/finance/widgets/finance_form_primitives.dart — FormCard/FieldRow/FieldDivider (03.1-08)"
  - "lib/core/utils/amount_parser.dart — parseAmountCentsOrNull (03.1-08)"
provides:
...
# Phase 03.1 Plan 14: Debt Form Screen Split Summary

**Split `debt_form_screen.dart` (327 → 150 lines) into `debt_form_logic.dart` (pure save-object builder), `debt_direction_toggle.dart`, and `debt_form_fields.dart`, adopting the shared FormCard/FieldRow/FieldDivider and amount parser from 3.1-08**
...
## Accomplishments
- Deleted the local `_FormCard`/`_FieldRow`/`_FieldDivider` classes from `debt_form_screen.dart` and switched to the shared `finance_form_primitives.dart` widgets (from 3.1-08).
- Replaced the inline amount-parsing block in `_save()` with `parseAmountCentsOrNull` (from 3.1-08's `amount_parser.dart`).
...
- **Applied the `_persist(Debt debt)` fallback pattern** named in the plan (matching sibling plans 3.1-12/3.1-13) to bring `debt_form_screen.dart` from 152 to exactly 150 lines after the primary extraction — unifying the `if (_isEditing) { await cubit.updateDebt(...) } else { await cubit.createDebt(...) }` branch into one ternary-returning helper method.
