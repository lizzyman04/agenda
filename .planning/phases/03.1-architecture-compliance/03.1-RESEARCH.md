# Phase 3.1: Architecture Compliance - Research
...
The second major finding is a **file-mapping conflict between plans 3.1-03 and 3.1-04**: `data/finance/finance_mappers.dart` (327 lines, in scope for 3.1-03) splits naturally into six per-entity mapper files, and those six files are exactly what 3.1-04's proposed `data/finance/<entity>/` subfolders need. Splitting the mapper file before the folder nesting avoids doing the same file twice. Recommended sequencing details are in the Sequencing section below.
...
**When to use:** All twelve remaining presentation screens (3.1-01, 3.1-02).
...
**When to use:** Any of the twelve screens that open a sheet or dialog for a sub-form (e.g. `debt_form_screen.dart`, `recurring_payment_form_screen.dart` if they have inline add/edit sheets — verify per-file during 3.1-02 planning).
...
(Phase 03 UAT test 5)."
```
Regression tests exist for both: `test/presentation/finance/budget_limit_sheet_test.dart`, `test/presentation/finance/goal_contribution_sheet_test.dart`. **Any 3.1-02 plan that touches a screen containing an inline sheet must check for this exact bug shape while splitting it out**, since splitting a screen is precisely the kind of edit that risks re-introducing it (moving controller creation to the wrong scope).
...
**What:** Dart's `part`/`part of` directive splits **one logical class** across multiple physical files that share the same library scope (so private members remain accessible across the split). This is the mechanism `isar_community_generator` already uses for `*.g.dart` files in this codebase (see e.g. `lib/data/finance/transaction_model.dart` → `part 'transaction_model.g.dart';`).
...
- **Splitting `data/finance/finance_mappers.dart` and moving files in the same commit**: do the split (3.1-03) and the folder move (3.1-04) as two mechanically distinct steps so `flutter analyze` isolates which change broke an import, if any.
...
| OS-registered state | None — no scheduled tasks, no pm2/systemd units (Phase 3.1 predates Phase 4's notification scheduling). | None |
...
// this constant resolves). Populate during 3.1-05 planning.
...
const lineLimitExemptions = <String, String>{
  ...
  //     'Flat ISO 4217 data table — see 3.1-RESEARCH.md',
};
...
2. **Should `presentation/finance/screens/` and `presentation/finance/widgets/` be split into per-entity subfolders (mirroring the proposed `data/finance/` and `domain/finance/` nesting) even though they're already under the 10-file cap?**
   ...
   - Recommendation: Treat as out of scope per the roadmap's own words — do not add this to any 3.1-0x plan unless the user explicitly asks for full presentation/finance parity with the goals slice's per-entity structure. Noted here only so the planner doesn't reintroduce it as accidental scope creep.
...
|-----------|----------|--------------------|--------------------|----------------|
| SC-1 | No hand-written file exceeds 150 lines | static check | `dart run tool/check_architecture.dart` | ❌ — build in 3.1-05 (Wave 0 gap) |
| SC-2 | No directory exceeds 10 hand-written files | static check | `dart run tool/check_architecture.dart` (same script, second check) | ❌ — build in 3.1-05 |
| SC-3 | Every presentation/application feature dir has README.md | static check | `dart run tool/check_architecture.dart` (same script, third check) | ❌ — build in 3.1-05 |
...
- **Per wave merge:** `dart run tool/check_architecture.dart` + `flutter test --no-pub --coverage` (full suite) — the guard script only becomes meaningful once 3.1-05 lands, so earlier waves rely on manual line-count spot checks (`wc -l <file>`) until the guard exists; see Sequencing below for why 3.1-05 is last.
...
- `.planning/ROADMAP.md` (Phase 3.1 section, full) and `.planning/STATE.md` — read in full, scope baseline and decision history
