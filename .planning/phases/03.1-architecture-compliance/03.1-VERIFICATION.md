...
overrides_applied: 0
human_verification:
  - test: "Cold start on a physical device / emulator against a database seeded before Phase 3.1 — open Tarefas, Resumo, Transações, Orçamentos, Dívidas, Recorrentes and Objetivos and confirm every pre-existing record is still visible."
    expected: "All previously saved tasks, transactions, budgets, debts, recurring payments and savings goals load exactly as before. No empty lists, no migration prompt, no Isar schema error."
    why_human: "03.1-17 relocated all six Isar model files and regenerated their schemas. Static evidence is strong (all six regenerated .g.dart files are byte-identical to their pre-move versions — collection name AND id hash unchanged — and di_test plus the full 265-test suite pass), but no emulator or device exists in the execution environment, so no cold start against real seeded data was ever performed for this phase."
  - test: "Manual smoke of the five extracted form screens: create AND edit one task (lib/presentation/tasks/form/), one transaction, one recurring payment, one debt and one savings goal. In each, exercise every picker (category, date, goal link, debt link, recurrence) and both the validation-failure path (empty amount, missing category) and the success path."
    expected: "Every field pre-fills correctly in edit mode, every picker opens and returns a value, validation snackbars appear with the same wording as before, and save closes the screen with the record persisted."
    why_human: "These five screens were decomposed into 30+ new files across Waves 1-2 and have NO widget tests. Their correctness currently rests entirely on `flutter analyze` plus the pre-existing suite, neither of which pumps these screens. Code-level diffs against the pre-refactor versions were checked and are equivalent, but UI wiring (controller ownership, sheet dismissal, setState routing) cannot be proven by grep."
  - test: "Open a task or finance form on a device and confirm the on-screen keyboard, focus order and scroll behaviour of the extracted field groups are unchanged."
    expected: "Tab/next moves through fields in the same order; the scroll view does not jump when the advanced-options card expands."
    why_human: "Visual/interaction quality — not observable from source."
---

# Phase 3.1: Architecture Compliance Verification Report
...
Phase 3.1 declares **no** requirement IDs (`**Requirements**: (none — internal quality)` in ROADMAP.md). No orphaned requirements are mapped to Phase 3.1 in REQUIREMENTS.md. Nothing to cover.
...
| `lib/config/di/infrastructure_module.dart` | 10 | `abstract class InfrastructureModule {}` — empty, and absent from the regenerated `injection.config.dart` | ℹ️ Info | Pre-existing DI scaffolding, not wired. Out of Phase 3.1 scope. |
...
2. **Highest residual risk is 03.1-17 (Isar model relocation).** Independently checked and **found to be lower than documented** — all six regenerated `.g.dart` files are byte-identical to their pre-move versions, so collection names *and* id hashes are provably unchanged. Cold start against real seeded data is still the honest gap.
