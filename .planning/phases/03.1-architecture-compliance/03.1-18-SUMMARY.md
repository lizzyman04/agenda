...
# Dependency graph
requires:
  - "03.1-01 (built the guard in informational mode and recorded the literal README-scope decision this plan implements)"
  - "03.1-02 through 03.1-17 (every file split and directory nesting — the final tree this plan documents and locks in)"
provides:
...
decisions:
  ...
  - "No raw column-width check was added. 88 raw lines in non-generated code exceed 80 characters while lines_longer_than_80_chars reports 0, because the lint exempts unsplittable tokens (mostly the long import URIs that 03.1-16/17's nesting created). A column-counting guard would flag 88 files the analyzer considers clean. Width policing defers to the lint."
...
# Phase 03.1 Plan 18: README Coverage and CI Enforcement Summary
...
`presentation/tasks/form/README.md` was stale: plans 03.1-02/03 and the
...
by 03.1-15) and pointers to the two new sub-READMEs; its controller-lifecycle
...
- **Issue:** The plan's `files_modified` lists only README files,
  ...
  The guard hardcoded `exit(0)` on failure (03.1-01's informational mode),
...
- **No raw column-width guard was added.** `lines_longer_than_80_chars`
  ...
  exempts unsplittable tokens — mostly the long `import` URIs 03.1-16/17
...
Documented exemptions in effect:
  - lib/core/constants/currencies.dart — Flat ISO 4217 data table (single static const List<Currency>) — splitting by line range fragments a single semantic unit and destroys Ctrl+F lookup for a currency code, for zero readability gain. See 3.1-RESEARCH.md Split-or-Exempt Recommendations.
...
have moved; but the phase-level advice from 03.1-16/17 stands: a human
on-device pass before merging Phase 3.1 as a whole is still worthwhile,
since the phase moved every finance and task file.

## Phase 03.1 Success Criteria — Final State

| SC | Criterion | State |
|----|-----------|-------|
| SC-1 | No hand-written file over 150 lines | Closed (03.1-02…15); guard reports zero LINES violations, one documented exemption (`currencies.dart`) |
| SC-2 | No directory over 10 hand-written `.dart` files | Closed (03.1-16/17); guard reports zero FILES violations |
...
change. The one threat in the plan's register (T-03.1-18-01, a CI
...
- `.planning/phases/03.1-architecture-compliance/03.1-18-SUMMARY.md`,
